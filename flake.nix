{
  ...
  outputs =
    ...
    let
      ...
      onchain = rec {
        projectFor = system: let
          ...
          fakeSrc = pkgs.runCommand "real-source-onchain" { } ''
            cp -rT ${self}/onchain $out
    ...
    in let
      onchain-scripts = system: (nixpkgsFor system).stdenv.mkDerivation {
        name = "onchain-scripts";
        src = self;
        buildInputs = [ ((onchain.projectFor system).flake {}).packages."dUSD-onchain:exe:scripts" ];
        doCheck = false;
        installPhase = ''
          scripts "$out"
          '';
      ...
      offchain = rec {
        projectFor = system: let
          ...
          fakeSrc = pkgs.runCommand "real-source-offchain" { } ''
            cp -rT ${self}/offchain $out
            chmod u+w $out/cabal.project
            cat $out/cabal-haskell.nix.project >> $out/cabal.project
          '';
          in
            (nixpkgsFor system).haskell-nix.cabalProject' {
              ...
              shell = {
                ...
                DUSD_SCRIPTS = onchain-scripts system;
              };
              sha256map = {
                "https://github.com/Plutonomicon/plutarch"."4052b285eb890799332c0cbe19cb08c1070f267a" = "sha256-8Tbrd9nPUkZarQiUTWYnNwbuz8wRhu+ipRK4XyedjTs=";
                "https://github.com/input-output-hk/plutus.git"."983e6af2154c4bdf86ed645062bcb62f304d0a4f" = "sha256-Ga+hIhrgq2kR5Vnso/Edo2wgQpFn167eWGl35oM093U=";
    ...
    in
      {
        onchain = {
          project = forAllSystems onchain.projectFor;
          flake = forAllSystems (system: (onchain.projectFor system).flake { });
        };

        offchain = {
          project = forAllSystems offchain.projectFor;
          flake = forAllSystems (system: (offchain.projectFor system).flake { });
        };
        ...
        in
          {
            ...
            offchain-test = {
              ...
              program = checkedShellScript system "dUSD-offchain-test"
                '' export DUSD_SCRIPTS=${onchain-scripts system}
                   cd ${self}
