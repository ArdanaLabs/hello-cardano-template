...
Anyone can trigger a surplus auction when the dUSD price module claim that the
...
Anyone can trigger a debt auction when the dUSD price module claim that the
...
\todo{Should the auctions be real auctions, discounted sales based on dUSD and
DANA price modules or OSMs, or should we have a list of trusted CEX/DEXes where
the transaction that triggers the auction, would sell the coins?}
