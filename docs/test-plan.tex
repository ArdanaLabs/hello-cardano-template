...
There is a minimum and maximum size for dUSD loans.
...
collateral in the vault at a $3\%$ discount compared to the market prices of
both ADA and dUSD.
...
In addition to the $3\%$ discount given to liquidators, the vault owner will be
charged a $10\%$ liquidation fee for allowing his vault to become unhealthy.
This means that only $90\%$ of the dUSD paid by the liquidators, is used to pay
back debt, burning the rest.
...
\begin{itemize}
  ...
  \item Users can deposit, withdraw, take out a loan and pay back a loan
    ...
    doesn't drop below the liquidation ratio\footnote{
      In the UX, the collateralization ratio is banned from dropping under the
  ...
  \item One can buy as much collateral as is needed to make the vault healthy
    again, and not more
  \item During liquidation, collateral is bought at a $3\%$ discount compared to
    the most current price from the price module
...
\todo{How much is the minimum loan (per loan, not per vault) and maximum amount
of collateral?}
