...
\begin{itemize}
  ...
  \item Off-chain code: PAB, tested through our own testing framework
    (Maeserat). This means that users will use the frontend to contact our
    remote wallet PAB\footnote{
...
The script will connect to its own contract instance of the dUSD admin PAB to
collect information on all dUSD vaults, and determine which ones are sick.
...
stored in the admin wallet and used for further protocol development.

\todo{Do we want to use these profits as revenue for Ardana, since it is
Ardana's bot?
Or do we want to put the profits in the buffer, or to buy DANA and reward
governance holders?}

\todo{Is there a more efficient way to discover all vaults?
They are all located on different addresses, depending on their vault owner's
public hash.
All vaults that have been used to take out a loan have connected to the price
module though, so we could collect the price module's history to collect the
vaults' information.}
