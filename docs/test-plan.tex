...
set at that timestamp.
The timestamp will be extracted from the transactions setting the stability fee
as the beginning of the ``txInfoValidRange'' range, where the on-chain validator
enforces this range to be less than $12$ hours long.
The reason for this measure is to make sure that stability fee calculations are
accurate even in vaults that haven't been updated for a long time.
