...
\begin{itemize}
  ...
  \item A vault is a UTXO which holds an NFT, as a unique ID, and the collateral
    assets
  \item The vault's datum consists of three pieces of information: A timestamp
    of its last update, the vault owner's public key, and the amount of dUSD
    debt (including stability fee) calculated at the last time of update
...
\todo{How will we extract timestamps from a transaction? \\

This is difficult for two reasons. Firstly, transactions don't know when they
get executed. The only time-related thing we have access to (by default) is a
time interval they were executed within. The second reason is that it is
possible a vault wasn't updated for a long time, and then the system's stability
fee changes, which can instantly change the debt amount substantially in
practice. \\

For the second point, we could store in the buffer a map from timestamp to
stability fee, rather than just changing the stability fee.
This would be a bit more complex to implement, since we need to prevent datum
overflow as well.}
