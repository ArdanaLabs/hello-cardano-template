...
main :: Effect Unit
main = do
  mcliPath <- lookupEnv "CLI_PATH"
  mfixturePath <- lookupEnv "FIXTURES_PATH"
  cliPath <- case mcliPath of
    Just cliPath -> pure cliPath
    Nothing -> throw "CLI_PATH not set"
  fixturePath <- case mfixturePath of
    Just cliPath -> pure cliPath
    Nothing -> throw "FIXTURES_PATH not set"
  launchAff_ $ do
    let cli = cliPath <> " "
    let jsonsPath = " " <> fixturePath <> "/jsons/"
